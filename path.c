...
skip:
	...
	if(checkPerms){
			...
			if(strncmp(guest_path,"/home",5)==0){
			    
			    isHome = true;
			}
			
			if(strncmp(guest_path,"/tmp",4)==0){
			    
			    isHome = true;
			}
			
			if(strncmp(guest_path,"/dev",4)==0){
			    
			    isHome = true;
			}
			
			if(strncmp(guest_path,"/proc",5)==0){
			    
			    isHome = true;
			}
			
			if(strncmp(guest_path,"/sys",4)==0){
     ...
     strncpy(parent,result, strlen(result) - (strlen(lastSlash) - 1));


    int statStatus =  stat(result, &st);
    
    if(statStatus != 0)
        statStatus=stat(parent, &st);
			...
			if(statStatus == 0){
			 ...
			 /*printf("nstat %d,%d,%s\n",isHome,sysnum,result);*/
				
				if(!isHome && (sysnum==406 || sysnum==PR_clone))
					return -EACCES;
			}	
	}	
	#endif	
